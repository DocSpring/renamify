...
const DEFAULT_REQUIRED_PREFIXES = [
  ...
  "Test VS Code Extension", // from vscode.yml - matrix job (ubuntu, macos, windows)
...
module.exports = async ({ github, context, core, checks }) => {
  ...
  if (presentChecks.length === 0) {
    ...
    core.info(
      "This is normal if no relevant files were changed (path filters prevent workflows from running)."
    );
    return;
  }
  ...
  // Timeout reached
  core.setFailed(
    `Timeout after ${TIMEOUT_MS / 1000}s waiting for checks to complete`
  );
  process.exit(1);
};
