...
fn test_gitignore_target_directory_is_never_scanned() {
    ...
    fs::write(
        repo_path.join("target/release/renamify"),
        "#!/bin/sh\necho renamify\n",
    )
    .unwrap();
    ...
    fs::write(
        repo_path.join("target/release/renamify.d"),
        "renamify: src/main.rs",
    )
    .unwrap();
    ...
    let plan = scan_repository(repo_path, "renamify", "renamed", &options).unwrap();

    // Verify that only src/main.rs was matched, not anything in target/
    // Note: src/main.rs has 2 occurrences of "renamify" in it
    assert_eq!(
        plan.matches.len(),
        2,
...
fn test_gitignore_with_unrestricted_level_3_still_respects_target() {
    ...
    // Create files
    fs::create_dir(repo_path.join("src")).unwrap();
    fs::write(repo_path.join("src/lib.rs"), "pub fn renamify() {}").unwrap();

    // Create target directory
    fs::create_dir_all(repo_path.join("target/debug")).unwrap();
    fs::write(
        repo_path.join("target/debug/renamify"),
        "binary content with renamify",
    )
    .unwrap();
...
fn test_git_repository_respects_gitignore() {
    ...
    // Create files
    fs::create_dir(repo_path.join("src")).unwrap();
    fs::write(repo_path.join("src/main.rs"), "fn renamify_function() {}").unwrap();

    // Create ignored files
    fs::create_dir(repo_path.join("target")).unwrap();
    fs::write(repo_path.join("target/renamify.exe"), "renamify binary").unwrap();
    fs::write(repo_path.join("renamify.log"), "renamify log file").unwrap();
