...
pub fn resolve_id(id: &str, operation: OperationType, refaktor_dir: &Path) -> Result<String> {
    if id == "latest" {
        resolve_latest_id(operation, refaktor_dir)
    } else {
...
fn resolve_latest_id(operation: OperationType, refaktor_dir: &Path) -> Result<String> {
    let history = History::load(refaktor_dir)?;
...
mod tests {
    ...
    fn test_resolve_specific_id_exists() {
        ...
        let result = resolve_id("abc123", OperationType::Undo, &refaktor_dir).unwrap();
        assert_eq!(result, "abc123");

        let result = resolve_id("def456", OperationType::Redo, &refaktor_dir).unwrap();
    ...
    fn test_resolve_specific_id_missing() {
        ...
        let result = resolve_id("missing", OperationType::Undo, &refaktor_dir);
    ...
    fn test_resolve_latest_for_undo() {
        ...
        let result = resolve_id("latest", OperationType::Undo, &refaktor_dir).unwrap();
    ...
    fn test_resolve_latest_for_redo() {
        ...
        let result = resolve_id("latest", OperationType::Redo, &refaktor_dir).unwrap();
        assert_eq!(result, "third");
    }
    ...
    fn test_resolve_latest_for_undo_only_reverts() {
        ...
        let result = resolve_id("latest", OperationType::Undo, &refaktor_dir);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    fn test_resolve_latest_for_redo_no_reverts() {
        ...
        let result = resolve_id("latest", OperationType::Redo, &refaktor_dir);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
