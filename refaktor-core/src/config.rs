...
impl Config {
    /// Load config from .refaktor/config.toml if it exists
    pub fn load() -> Result<Self> {
        if let Ok(cwd) = std::env::current_dir() {
            let config_path = cwd.join(".refaktor").join("config.toml");
    ...
    /// Save config to .refaktor/config.toml
    pub fn save(&self) -> Result<()> {
        let cwd = std::env::current_dir()?;
        let config_dir = cwd.join(".refaktor");
        let config_path = config_dir.join("config.toml");

        // Create .refaktor directory if it doesn't exist
