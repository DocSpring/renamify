use refaktor_core::{apply_plan, ApplyOptions, Plan, MatchHunk, Rename, RenameKind, Stats};
...
fn test_apply_content_edits() {
    ...
    fs::write(&test_file, "fn old_name() {\n    println!(\"old_name\");\n}").unwrap();
...
fn test_apply_renames() {
    ...
    assert!(temp_dir.path().join("new_name_dir").join("inner.txt").exists());
