...
#[test]
fn test_rfignore_respected_by_default() {
    ...
    // Create .rfignore file that ignores ignored.txt
...
#[test]
fn test_rfignore_with_patterns() {
    let temp_dir = TempDir::new().unwrap();
    ...
    // Create .rfignore that ignores build directory
...
#[test]
fn test_rfignore_with_unrestricted_level() {
    let temp_dir = TempDir::new().unwrap();
    ...
    // Create .rfignore file
    fs::write(temp_dir.path().join(".rfignore"), "ignored.txt").unwrap();

    // Test with unrestricted level 0 (default - respects .rfignore)
...
#[test]
fn test_rfignore_in_subdirectory() {
    let temp_dir = TempDir::new().unwrap();
    ...
    // Create .rfignore in subdirectory
...
#[test]
fn test_rfignore_combined_with_gitignore() {
    let temp_dir = TempDir::new().unwrap();
    ...
    .unwrap();
    fs::write(
        temp_dir.path().join("rf_ignored.txt"),
        "old_name rf ignored",
    )
    .unwrap();
    fs::write(
