...
// Mock RenamifyService
vi.mock('./renamify-service');

describe('RenamifyTools', () => {
  let tools: RenamifyTools;
  let mockService: ReturnType<typeof vi.mocked<RenamifyService>>;

  beforeEach(() => {
    mockService = new RenamifyService() as ReturnType<
      typeof vi.mocked<RenamifyService>
    >;
    tools = new RenamifyTools(mockService);
    vi.clearAllMocks();
  });
  ...
  describe('plan', () => {
    ...
    it('should create plan and add helpful context', async () => {
      ...
      expect(result).toContain("Use 'renamify_apply' to apply these changes");
  ...
  describe('history', () => {
    it('should show history with usage hint', async () => {
      ...
      expect(result).toContain("Use 'renamify_undo <id>' to revert");
    });
  ...
  describe('status', () => {
    it('should show status with action hints', async () => {
      ...
      expect(result).toContain("Use 'renamify_apply' to apply pending plans");
    });
