...
<CardGrid>
  ...
  <Card title="Content & Files" icon="document">
    Replaces file contents and renames files/directories in a single operation.
  </Card>
...
- **More case styles**: Handles snake_case, kebab-case, camelCase, PascalCase,
  SCREAMING_SNAKE_CASE, Title Case, Sentence case, Train-Case, SCREAMING-TRAIN-CASE,
  dot.case, flatcase, UPPERFLATCASE, and space-separated styles
...
Renamify respects standard ignore files:

- `.gitignore` - Git's ignore patterns
- `.ignore` - Generic ignore file (used by ripgrep, fd, etc.)
- `.rnignore` - Renamify-specific patterns

It also:

- **Includes** hidden files like `.goreleaser.yaml` and `.github/` by default
- Always excludes the `.git` directory (never scanned or renamed)
- Skips binary files (unless using `-uuu` flag)
...
Renamify is developed and maintained by [DocSpring](https://docspring.com?utm_source=renamify), a
company that builds a PDF template editor and an API for generating PDFs. It's open source under
the MIT license.
