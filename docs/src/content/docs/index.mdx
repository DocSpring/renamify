...
Smart search & replace for code and files with <br/>**case-aware transformations** 
and **built-in undo/redo**.
</span>
<span style="font-size: 1.35rem;">
An **MCP Server**, **CLI tool**, and **VS Code / Cursor Extension** _(coming soon)_
</span>
...
<LinkCard
  ...
  description="Connect Renamify to Claude, Cursor, and other AI tools via the Model Context Protocol (MCP) for intelligent, automated refactoring."
/>
...
Renamify is a command-line tool that understands different naming conventions and can rename both file contents and the files themselves in a single atomic operation. It's designed for developers who need to refactor codebases safely and efficiently.
...
See the [installation guide](/renamify/installation) for system-wide installation and more options.
...
1. **AI Development Era**: With AI assistants like Claude and Cursor, many developers now code outside traditional IDEs. These AI tools need programmatic access to refactoring capabilities.
...
In the new era of AI development, productivity has skyrocketed, but AI agents are still missing a lot of tooling to make them more effective. Renamify helps fill one of those gaps.
...
VS Code introduced [case-preserving find and replace](https://devblogs.microsoft.com/visualstudio/keep-your-casing-with-case-preserving-find-and-replace/), which is great for simple cases. Renamify goes much further:
...
- Preserves acronyms during transformation: `parseURL` → `parseWebAddress` (if renaming URL to WebAddress)
...
<details>
<summary>Can I use Renamify in CI/CD pipelines?</summary>

Absolutely! Renamify is designed for automation:

```bash
# Check for naming consistency
renamify plan oldConvention newConvention --dry-run

# Automated refactoring with safety
renamify rename oldName newName --atomic --commit

# Verify no uncommitted renames
renamify status --check
```
...
Yes! Renamify is cross-platform and works on:
- Linux (x86_64, arm64)
- macOS (Intel, Apple Silicon)
- Windows (native builds coming soon, build from source available now)

All features work consistently across platforms, including proper handling of Windows path separators.
...
Renamify is developed and maintained by [DocSpring](https://docspring.com), a company that builds developer tools and PDF API services. It's open source under the MIT license.
