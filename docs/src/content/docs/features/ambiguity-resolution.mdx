...
:::note These conventions only apply when there's actual ambiguity. If an
identifier already has a clear case style then it's always preserved. :::

### 2. File Context Analysis

First, Renamify analyzes the predominant case style in your file. If a file uses
...
- `.config` would become `.configData` (camelCase) even if the replacement is
  `config_data`
...
This intelligent system ensures that Renamify makes sensible decisions about 95%
of the time, while still giving you full control when needed.
