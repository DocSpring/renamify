...
In addition to replacing text within files, Renamify can rename files and
directories that match your pattern.
...
1. Files first
2. Then directories, deepest first This prevents conflicts and ensures all
   operations succeed.

## Root Directory Renaming

**NEW**: Renamify can now rename the project root directory itself when the
directory name matches your pattern.
...
- **Next steps snippet**: Provides instructions for rebuilding/reloading after
  root rename
