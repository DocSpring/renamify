...
### Basic Rename
```bash
# Rename with table preview and confirmation
...
refaktor rename oldName newName --include "**/*.{ts,tsx}" --preview table
```
...
refaktor rename old_function new_function --commit --preview diff
```
