...
```bash
refaktor apply [ID] [OPTIONS]
```

## Arguments

- `[ID]` - Plan ID, file path, or "latest" (optional - defaults to .refaktor/plan.json)
...
### Basic Apply
```bash
# Apply the default plan at .refaktor/plan.json
refaktor apply

# Explicitly apply the latest/default plan
refaktor apply latest
```

### Apply by Plan ID
```bash
# Apply a specific plan from history
refaktor apply abc123def456

# Apply from a file path
refaktor apply ./my-refactor-plan.json

# Apply from a custom location
refaktor apply ./backups/old-plan.json
```
...
refaktor apply abc123def456 --commit
```
