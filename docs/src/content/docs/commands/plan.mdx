...
- `<OLD>` - The text pattern to find and replace
...
### Diff Format
```bash
refaktor plan getUserName fetchUserProfile --preview-format diff
```
...
### JSON Format
```bash
refaktor plan getUserName fetchUserProfile --preview-format json
```

Outputs structured JSON for programmatic use:
```json
{
  "id": "abc123-20241201-143022",
...
### Basic Planning
```bash
# Create a plan with table preview
...
refaktor plan old_name new_name --dry-run --preview-format table
```
...
### Specific File Types
```bash
# Only plan changes for Rust files
