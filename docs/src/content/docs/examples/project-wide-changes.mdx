...
```bash
refaktor rename mylib awesome_lib --commit
```
...
refaktor rename old_name new_name src/ lib/
```

### Use Dry Run First
```bash
# Explore the scope before committing
refaktor dry-run old_name new_name src/
...
refaktor plan large_change huge_change --preview json > plan.json
