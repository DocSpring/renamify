...
### Use Dry Run To Avoid Plan Creation

```bash
# Explore the scope before committing
renamify plan old_name new_name src/ --dry-run

# Or with patterns
renamify plan old_name new_name --include "src/**" --dry-run

# Count potential matches
renamify plan old_name new_name --dry-run | grep -c "│"
```

### JSON Output for Scripting

```bash
# Display JSON output to terminal for scripting/piping
renamify plan large_change huge_change --output json | jq '.stats'

# Note: The plan is always saved to .renamify/plan.json regardless of output format
# Apply the saved plan later
renamify apply
```
