...
description:
  Watch Renamify rename itself, run the test suite using the new name, then
  rename itself back
---
...
>
  In computer programming, self-hosting is the use of a program as part of the
...
Watch Renamify rename itself to `awesome_file_renaming_tool`, then use the new
binary to rename itself back again.
...
<LinkCard
  ...
  href="https://github.com/DocSpring/renamify/blob/main/scripts/e2e-test.sh"
