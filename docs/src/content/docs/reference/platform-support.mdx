...
Refaktor is designed to work consistently across Linux, macOS, and Windows in the near future.
...
### macOS (APFS/HFS+)
Most macOS systems use case-insensitive filesystems. Refaktor handles this by:
...
### Windows (NTFS)
Windows filesystems are typically case-insensitive. Refaktor will provide:
...
### Path Separators
Refaktor accepts both forward slashes and backslashes:

```bash
# These are equivalent on Windows
refaktor plan old new --include "src/**/*.js"
refaktor plan old new --include "src\\**\\*.js"
```
...
```bash
# Refaktor detects and warns about long paths
...
### Unicode Support
Refaktor fully supports Unicode filenames and content:

```bash
# Works with international characters
refaktor rename 用户 profile --preview table
refaktor rename josé maria --preview table
```
...
- Refaktor handles gracefully without errors

## File Permissions

### Unix-like Systems (Linux/macOS)
```bash
# Refaktor respects and preserves file permissions
...
### Platform Detection
Refaktor automatically detects platform capabilities:

```bash
# Force platform behavior (rarely needed)
REFAKTOR_PLATFORM=linux refaktor plan old new    # Force Linux behavior
REFAKTOR_PLATFORM=windows refaktor plan old new  # Force Windows behavior
```

### File System Override
```bash
# Force case sensitivity detection (expert use only)
REFAKTOR_CASE_SENSITIVE=true refaktor plan old new
REFAKTOR_CASE_SENSITIVE=false refaktor plan old new
```
...
refaktor rename oldName OldName  # Works directly

# Use parallel processing for large projects
refaktor plan old new --include "**/*.rs"  # Fast on Linux
```

### macOS
```bash
# Be aware of case insensitivity
refaktor rename oldName OldName --preview table  # Shows two-step rename

# Consider filesystem overhead for huge projects
refaktor plan old new --include "src/**" --exclude "node_modules/**"
```
...
refaktor plan old new --exclude "**/very/deeply/nested/directories/**"
```
...
refaktor rename old new --preview table
