...
pub fn handle_rename(
    ...
    let (root_renames, other_renames): (Vec<_>, Vec<_>) = plan.renames
        .into_iter()
        .partition(|rename| {
    ...
    plan.renames = if rename_root {
        ...
        root_renames.clone().into_iter().chain(other_renames).collect()
...
fn print_root_rename_snippet(root_renames: &[refaktor_core::scanner::Rename]) -> Result<()> {
    ...
    let old_name = rename.from.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("project");
    let new_name = rename.to.file_name()
        .and_then(|n| n.to_str())
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        println!("parent=\"$(dirname \"$PWD\")\"; old=\"$(basename \"$PWD\")\"; new=\"{}\"", new_name);
